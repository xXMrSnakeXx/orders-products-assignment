...
import { Order, OrderInput, OrderWithProducts } from '../types.js';

export const getAllOrdersWithProducts = async (
  req: Request,
  res: Response<OrderWithProducts[] | { message: string }>
) => {
  try {
    ...
    const ordersMap = new Map<number, OrderWithProducts>();
